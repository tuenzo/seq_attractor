...
if [ $? -eq 0 ]; then
    ...
    echo "  2. 复制为基线文件:"
    echo "     cp test_reports/baseline_before_optimize_*.json test_reports/baseline_before_optimize.json"
    echo "  3. 切换回优化分支（如 optimize/blas）:"
    echo "     git checkout optimize/blas"
    echo "  4. 运行对比:"
    echo "     python scripts/auto_test.py --compare --baseline-file test_reports/baseline_before_optimize.json"
else
    echo ""
    echo "基线测试失败，请检查错误信息"
    exit 1
